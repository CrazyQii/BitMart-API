...
class OkexPublic(object):
    ...
    def get_kline(self, symbol, time_period = 200, interval= 1):
        end = datetime.datetime.utcnow()
        start = end - datetime.timedelta(seconds = time_period*60)
        url = self.urlbase + "api/spot/v3/instruments/%s/candles?granularity=%s&start=%sZ&end=%sZ" % ('-'.join(symbol.split("_")), interval*60,start.isoformat(), end.isoformat())
        is_ok, okex_content = self.request("GET", url)
        content = []
        for line in okex_content:
            content.append({
                "timestamp": int(time.mktime((datetime.datetime.strptime(line[0], "%Y-%m-%dT%H:%M:%S.%fZ")).timetuple())),
                "open": line[1],
                "high": line[2],
                "low": line[3],
                "close": line[4],
