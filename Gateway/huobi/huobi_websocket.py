...
class HuobiWs(Thread):
    ...
    def _sign_message(self, params):
        try:
            ...
            logging.error(f'Huobi sign message error: {e}')
        ...
        while True:
            try:
                if private:
                    ...
                    logging.info('Huobi start to connect ' + self.urlbase)
                    ws = websocket.create_connection(self.urlbase)
            except Exception as e:
                logging.error(f'Huobi connect error: {e}')
                times = times + 1
                time.sleep(3)
                if times >= 5:
                    logging.error(f'Huobi has connected {times} times, and connection end!!!')
                    break
            else:
                break
        return ws
    ...
    def _stop(self, channel):
        try:
            ...
            logging.error(f'Huobi stop channel error: {e}')
    ...
    def _sub(self, channel, private=False):
        try:
            ...
            logging.error(f'Huobi start channel error: {e}')
    ...
    def _get_kline(self, params: dict, limit=500):
        try:
            ...
            logging.error(f'Huobi get kline error: {e}')
    ...
    def _get_trade(self, params: dict):
        try:
            ...
            logging.error(f'Huobi get trade error: {e}')
    ...
    def _get_price(self, params: dict):
        try:
            ...
            logging.error(f'Huobi get price error: {e}')
    ...
    def _get_orderbook(self, params: dict):
        try:
            ...
            logging.error(f'Huobi get orderbook error: {e}')
    ...
    def _get_order(self, params: dict, status=None, limit=200):
        ...
        try:
            ...
            logging.error(f'Huobi get order error: {e}')
    ...
    def _get_wallet_balance(self, params: dict):
        try:
            ...
            logging.error(f'Huobi get wallet balance error: {e}')

    def on_message(self):
        def _message():
            try:
                if self.ws is None:
                    logging.error('Huobi public does not connected, and connection closed!')
                    return
                else:
                    while True:
                ...
                logging.error(f'Huobi Sub public error {e} : try to connect again')
        ...
        def _auth_message():
            try:
                if self.wss is None:
                    logging.error('Okex auth does not connected, and connection closed!')
                    return
                while True:
                ...
                logging.error(f'Huobi Sub auth error {e} : try to connect again')
